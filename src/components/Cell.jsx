...
const Cell = ({ isAlive, toggleCellState, heatmapValue, showHeatmap }) => {
  ...
  const getColorFromHeatmapValue = (value) => {
    ...
    const keys = Object.keys(spectrum).map(Number).sort((a, b) => a - b);
    const key = keys.reduce((prevKey, currKey) => currKey <= value ? currKey : prevKey, 0);
  ...
  const cellStyle = {
    ...
    backgroundColor: showHeatmap ? getColorFromHeatmapValue(heatmapValue) : (isAlive ? 'black' : 'white'),
